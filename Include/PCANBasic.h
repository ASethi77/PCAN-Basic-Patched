...
#define TPCANHandle                   uint16_t   // Represents a PCAN hardware channel handle
#define TPCANStatus                   uint32_t  // Represents a PCAN status/error code
#define TPCANParameter                uint8_t   // Represents a PCAN parameter to be read or set
#define TPCANDevice                   uint8_t   // Represents a PCAN device
#define TPCANMessageType              uint8_t   // Represents the type of a PCAN message
#define TPCANType                     uint8_t   // Represents the type of PCAN hardware to be initialized
#define TPCANMode                     uint8_t   // Represents a PCAN filter mode
#define TPCANBaudrate                 uint16_t   // Represents a PCAN Baud rate register value
#define TPCANBitrateFD                char*  // Represents a PCAN-FD bit rate string
#define TPCANTimestampFD              uint64_t // Represents a timestamp of a received PCAN FD message
...
typedef struct tagTPCANMsg
{
    ...
    uint8_t              LEN;     // Data Length Code of the message (0..8)
...
{
    uint32_t  millis;             // Base-value: milliseconds: 0.. 2^32-1
...
typedef struct tagTPCANMsgFD
{
    uint32_t             ID;       // 11/29-bit message identifier
    TPCANMessageType  MSGTYPE;  // Type of the message
    uint8_t              DLC;      // Data Length Code of the message (0..15)
    uint8_t              DATA[64]; // Data of the message (DATA[0]..DATA[63])
} TPCANMsgFD;
...
typedef struct tagTPCANChannelInformation
{
    ...
    uint8_t controller_number;                     // CAN-Controller number
    uint32_t device_features;                      // Device capabilities flag (see FEATURE_*)
    char device_name[MAX_LENGTH_HARDWARE_NAME]; // Device name
    uint32_t device_id;                            // Device number   
    uint32_t channel_condition;                    // Availability status of a PCAN-Channel          
