...
begin
  ...
  begin
    ...
    stsResult := TPCANBasic.GetValue(TPCANBasic.PCAN_NONEBUS, PCAN_ATTACHED_CHANNELS, PTPCANChannelInformation(Info), sizeof(TPCANChannelInformation) * iChannelsCount);
    if (stsResult = PCAN_ERROR_OK) then
    begin
...
begin
	...
	case cbbParameter.ItemIndex of
        ...
        begin
	    	...
	    	stsResult := TPCANBasic.SetValue(m_PcanHandle, PCAN_DEVICE_ID, PLongWord(@iBuffer), sizeof(iBuffer));
		    if(stsResult = PCAN_ERROR_OK) then
...
begin
	...
	case cbbParameter.ItemIndex of
	    ...
	    0:
        begin
		    stsResult := TPCANBasic.GetValue(m_PcanHandle, PCAN_DEVICE_ID, PLongWord(@iBuffer), sizeof(iBuffer));
    		if(stsResult = PCAN_ERROR_OK) then
    		begin
